...
const TRIAL_DURATION_MS = TRIAL_CONFIG.durationMs;
...
export function setTrialData(data: TrialData): void {
  ...
  // Set a cookie so middleware / SSR can read the token. Secure is required
  // when the page is served over HTTPS; falling back to omitting it on
  // localhost (http:) so dev works without a TLS proxy.
  const maxAge = Math.floor(TRIAL_DURATION_MS / 1000);
  const secure = window.location.protocol === 'https:' ? '; Secure' : '';
  document.cookie = `${TRIAL_COOKIE}=${data.token}; path=/; max-age=${maxAge}; SameSite=Lax${secure}`;
}
...
export function clearTrialData(): void {
  ...
  const secure = window.location.protocol === 'https:' ? '; Secure' : '';
  document.cookie = `${TRIAL_COOKIE}=; path=/; max-age=0; SameSite=Lax${secure}`;
}
 ...
 */
export async function trialApiFetch(path: string, init: RequestInit = {}): Promise<Response> {
  const apiUrl = process.env.NEXT_PUBLIC_API_URL;
  const data = getTrialData();
