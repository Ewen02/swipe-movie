...
import {
  ...
  Home, Zap, Trophy, ArrowUpRight, ArrowDownRight,
} from "lucide-react"
...
const KPI_CONFIG = [
  { key: "totalUsers", label: "Total Users", icon: Users, color: "bg-blue-500/20 text-blue-400", sparkColor: "#3b82f6" },
  { key: "activeToday", label: "Actifs Aujourd'hui", icon: Activity, color: "bg-green-500/20 text-green-400", sparkColor: "#22c55e" },
  { key: "activeWeek", label: "Actifs 7j", icon: TrendingUp, color: "bg-emerald-500/20 text-emerald-400", sparkColor: "#10b981" },
  { key: "activeMonth", label: "Actifs 30j", icon: Calendar, color: "bg-purple-500/20 text-purple-400", sparkColor: "#a855f7" },
  { key: "totalRooms", label: "Rooms", icon: Home, color: "bg-orange-500/20 text-orange-400", sparkColor: "#f97316" },
  { key: "totalSwipes", label: "Swipes", icon: Zap, color: "bg-pink-500/20 text-pink-400", sparkColor: "#ec4899" },
  { key: "totalMatches", label: "Matches", icon: Trophy, color: "bg-amber-500/20 text-amber-400", sparkColor: "#f59e0b" },
] as const
...
export function KPICards({ stats, activity }: KPICardsProps) {
  ...
  return (
    <motion.div variants={fadeInUp} className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
      {KPI_CONFIG.map((cfg) => {
