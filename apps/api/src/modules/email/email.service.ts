...
export class NestEmailService {
    ...
    this.emailService = new EmailService(
      {
        ...
        fromEmail:
          ...
          'noreply@swipe-movie.com',
        fromName:
          this.configService.get<string>('EMAIL_FROM_NAME') || 'Swipe Movie',
        baseUrl:
  ...
  async sendRoomInvite(
    ...
    if (!result.success) {
      this.logger.error(
        `Failed to send room invite to ${email}: ${result.error}`,
      );
    }
  ...
  async sendMatchNotification(
    ...
    if (!result.success) {
      this.logger.error(
        `Failed to send match notification to ${email}: ${result.error}`,
      );
    }
    return result.success;
  }
  ...
  async sendWeeklyDigest(
    ...
    if (!result.success) {
      this.logger.error(
        `Failed to send weekly digest to ${email}: ${result.error}`,
      );
    }
    return result.success;
  }
